...
contract Script is OpsMultisigScript {
    function run(bool shouldSend) public override {
        ...
        uint256 totalRewardsToDistribute = 2_400_000 ether;
        ...
        // 1: Curve YFI-ETH (v2) 100,000 / 4 $COVE
        // 2: Curve dYFIETH-f (v2)	150,000 / 4 $COVE
        // 3: LP Yearn CRV (v2) 300,000 / 4 $COVE
        // 4: LP Yearn PRISMA (v2) 50,000 / 4 $COVE
        // 5: USDC (v3)	500,000 / 4 $COVE
        // 6: DAI (v3)	150,000 / 4 $COVE
        // 7: WETH (v3)	100,000 $COVE
        // 8: coveYFI/YFI 100,000 / 4 $COVE
        // 9: DAI-2 200,000 / 4 $COVE
        // 10: WETH-2 50,000 / 4 $COVE
        // 11: crvUSD-2 50,000 / 4 $COVE
        // $coveYFI	600,000 / 4 $COVE

        uint256[] memory rewardAmounts = new uint256[](12);
        rewardAmounts[0] = 50_000 ether;
        rewardAmounts[1] = 100_000 ether;
        rewardAmounts[2] = 150_000 ether;
        rewardAmounts[3] = 300_000 ether;
        rewardAmounts[4] = 50_000 ether;
        rewardAmounts[5] = 500_000 ether;
        rewardAmounts[6] = 150_000 ether;
        rewardAmounts[7] = 100_000 ether;
        rewardAmounts[8] = 100_000 ether;
        rewardAmounts[9] = 200_000 ether;
        rewardAmounts[10] = 50_000 ether;
        rewardAmounts[11] = 50_000 ether;
        uint256 coveYFIRewardAmount = 600_000 ether;
        ...
        // Execute batch
        if (shouldSend) executeBatch(true, 34);
    }
}
