...
contract Script is OpsMultisigScript {
    function run(bool shouldSend) public override {
        ...
        // queueing up the rewards for epoch 2, week 1
        // 2.8M / 4 COVE to CoveYFIRewardsGauge
        // (600K / 5) / 4 COVE to each V2 auto-compounding gauge
        // (600k / 3) / 4 COVE to each V3 auto-compounding gauge
        uint256 balanceBefore = IERC20(coveToken).balanceOf(coveYfiRewardsGauge);
        addToBatch(
            coveToken,
            0,
            abi.encodeCall(IERC20.transfer, (coveYfiRewardsGaugeRewardForwarder, uint256(2_800_000 ether) / 4))
        );
        addToBatch(
            coveYfiRewardsGaugeRewardForwarder, 0, abi.encodeCall(RewardForwarder.forwardRewardToken, (coveToken))
        );
        require(
            IERC20(coveToken).balanceOf(coveYfiRewardsGauge) == balanceBefore + uint256(2_800_000 ether) / 4,
        ...
        for (uint256 j = 0; j < info.length; j++) {
            ...
            addToBatch(
                ...
                abi.encodeCall(
                    IERC20.transfer,
                    (autoCompoundingGaugeRewardForwarder, (uint256(600_000 ether) / rewardSplitAmount) / 4)
                )
            );
            addToBatch(
            ...
            require(
                IERC20(coveToken).balanceOf(info[j].autoCompoundingGauge)
                    == balanceBefore + (uint256(600_000 ether) / rewardSplitAmount) / 4,
