...
contract ReadOnlyGlobalDeployer is ReadOnlyDeployer {
    // --------------------------------------------------------------------------------------------
    // Constants
    // --------------------------------------------------------------------------------------------
    Vm constant vm = Vm(address(bytes20(uint160(uint256(keccak256("hevm cheat code"))))));
    ...
    function init() external {
        ...
        string memory path =
            string.concat(root, "/lib/cove-contracts-boosties/deployments/", deploymentContext, "/.chainId");
    ...
    //     // then the sync step can read it to get more info about the deployment, including the exact source,
    // metadata....
    //     return "";
    // }
    ...
    function _getDeploymentContext() private returns (string memory context) {
        ...
        if (bytes(context).length == 0) {
            ...
            if (currentChainID == 1337 || currentChainID == 31_337) {
                context = "void";
            } else {
    ...
    function _getExistingDeploymentAdress(string memory name) internal view returns (address payable) {
        string memory root = vm.projectRoot();
        string memory path =
            string.concat(root, "/lib/cove-contracts-boosties/deployments/", deploymentContext, "/", name, ".json");
...
function getDeployer() returns (ReadOnlyDeployer) {
    address addr = 0x666f7267652d6465706C6f790000000000000000;
    if (addr.code.length > 0) {
        return ReadOnlyDeployer(addr);
    }
    Vm vm = Vm(address(bytes20(uint160(uint256(keccak256("hevm cheat code"))))));
    bytes memory code = vm.getDeployedCode("ReadOnlyDeployer.s.sol:ReadOnlyGlobalDeployer");
    vm.etch(addr, code);
    vm.allowCheatcodes(addr);
    ReadOnlyGlobalDeployer deployer = ReadOnlyGlobalDeployer(addr);
    deployer.init();
    return deployer;
}
